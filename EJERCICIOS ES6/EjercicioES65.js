...
const overage = ages1.filter(age => age > 18);
console.log(overage);
...
const strumers = streamers2.filter(streamer => streamer.name.includes('u'));
...
const streamers3 = streamers2.filter(streamer => {
    if (streamer.gameMorePlayed.includes('Legends')) {
        if (streamer.age > 35) {
            streamer.gameMorePlayed = streamer.gameMorePlayed.toUpperCase();
        } else {
            streamer.gameMorePlayed = streamer.gameMorePlayed;
        }
        return true;
    } else {
        return false;
